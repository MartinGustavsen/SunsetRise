...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased" style="background-color:#aaaaaa;">
        <div class="flex-center position-ref full-height">
            <div class="container">
                <h1 class="text-center">Sunrise & Sunset</h1>
                ...
                <div class="row">
                    <div class="offset-2 col-4">
                        @isset($sun_times)                        
                            <p>Time for sunrise and sunset at {{$location}}</p>
                            <table class="table table-striped table-bordered table-sm">
                                ...
                                <tbody>
                                    @foreach ($sun_times as $sun_time)
                                        <tr>
                                            <td>{{$sun_time['date']}}</td>
                                            <td>{{$sun_time['sunrise']}}</td>
                                            <td>{{$sun_time['sunset']}}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        @endisset
                        @isset($locations) 
                            @foreach ($locations as $location_format)
                                <form action="/" method="get">
                                    <div class="form-group">
                                        <input type="hidden" id="location_id" name="location" value={{$location_format}}>
                                        <button type="submit" class="btn btn-primary hyperlink">{{$location_format}}</button>
                                    </div>
                                </form>
                            @endforeach
                        @endisset       
